...
namespace Menu
{
    ...
    std::string menuSelectComputer(std::string typeOfComputer)
    {
        ...
        if (SIZE == 1)
        {
            ConsoleUtils::print("Loại máy này đã hết", {Constants::ANSI::Foreground::YELLOW});
            Menu::button(5, 1, "ok");
            return "";
        }
        while (true)
    ...
    std::string menuSelectComputerRegistered()
    {
        ...
        if (SIZE == 1)
        {
            ConsoleUtils::print("Không có máy nào được đăng ký", {Constants::ANSI::Foreground::YELLOW, Constants::ANSI::Style::BOLD});
            Menu::button(10, 1, "ok");
            return "";
        }
        while (true)
